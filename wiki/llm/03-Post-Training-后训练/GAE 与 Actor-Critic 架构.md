...
关联阅读：可结合 [[10-LLM-大语言模型/03-Post-Training-后训练/PPO (近端策略优化)|PPO (近端策略优化)]] 理解；PPO 的优势估计通常依赖 GAE 和 Actor-Critic 框架。
...
- **前置**：[[10-LLM-大语言模型/03-Post-Training-后训练/PPO (近端策略优化)|PPO (近端策略优化)]] — PPO 的优势估计通常依赖 GAE 和 Actor-Critic 框架。
- **对比**：[[10-LLM-大语言模型/03-Post-Training-后训练/GRPO (组相对策略优化)|GRPO (组相对策略优化)]] — GRPO 通过组相对优势弱化对独立 Critic 的依赖。
